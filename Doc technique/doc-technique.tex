...
Ces deux fonctions initialisent les paramètres nécessaires et lance l'exécution respective des algorithmes NegaMax et AlphaBeta dans l'ordre suivant :
\begin{itemize}
...
\begin{itemize}
	...
	\item[\textbullet] Les coordonnées (xSelect, ySelect) de la case sélectionnée si le coup en cours de calcul est un coup à plusieurs prises : le joueur ne peut pas désélectionner son pion et le calcul des mouvements doit se faire depuis ce pion
\end{itemize}
...
\begin{itemize}
	...
	\item[\textbullet] La fonction retourne la meilleure valeur de fonction de coût issue de l'appel récursif de tous ses fils
\end{itemize}
...
\begin{itemize}
	...
	\item[\textbullet] Ismaxprec : il s'agit d'un booléen indiquant si la valeur maxprec est à prendre en compte, celle-ci n'est pas à prendre en compte lorsque le n{\oe}ud père est le premier fils de ses frères car il n'y a alors pas encore de valeur maximal vu que la première est en cours de calcul via le n{\oe}ud actuel
\end{itemize}
