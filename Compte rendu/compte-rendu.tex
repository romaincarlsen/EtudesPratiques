...
\FigureEPS{h,t,b,p}{8.5cm}{./img/mvc.eps}{Schéma modèle MVC (Model View Controller)}{fig:mvc}

\begin{itemize}
	\item[\textbullet] \underline{Modèle :}
...
Classes Game, Checkerboard et Player (voir partie II - Le jeu)
	...
	\item[\textbullet] \underline{Contrôleur :}
...
\FigureEPS{h,t,b,p}{8.5cm}{./img/diagramme.eps}{Diagramme de classe}{fig:diag}
...
\FigureEPS{h,t,b,p}{1.2\textwidth}{./img/screen.eps}{Jeu de dames}{fig:dames}
...
\FigureEPS{h,t,b,p}{8.5cm}{./img/plateau.eps}{Damier correspondant à la fonction de coût}{fig:coût}
...
\begin{itemize}
	...
	\item[\textbullet] Depth = il s'agit de la profondeur restant à parcourir.
	\item[\textbullet] Color =  il s'agit du joueur à qui c'est le tour de jeu = blanc/noir = 1/-1.
	\item[\textbullet] Node is a terminal node = lorsqu'un n{\oe}ud est une feuille de l'arbre = lorsque la profondeur maximale a été atteinte ou que le damier admet un gagnant ou une égalité.
	\item[\textbullet] The heuristic value of node = la valeur de la fonction de coût.
	\item[\textbullet] BestValue = la valeur de fonction de coût maximal actuellement trouvée parmi les n{\oe}uds fils.
	...
	\item[\textbullet] val := -negamax(child, depth - 1, -color) = appel récursif de la fonction NegaMax pour chaque fils et inversement de la valeur de retour pour adapter la valeur au fait que le n{\oe}ud actuelle est un joueur différent.
	\item[\textbullet] bestValue := max( bestValue, val ) =  sélection de la valeur maximale.
...
\begin{itemize}
\item[\textbullet] La première a été de créer un fichier texte contenant la taille du plateau et, pour chaque n{\oe}ud, le contenu du damier, le temps d'exécution, sa valeur, son nombre de n{\oe}ud fils et le nombre de noeud analysé.
\item[\textbullet] La deuxième est de créer un reporting graphique, codé en javascript via Canvas, et permettant de mieux visualiser les situations.
\end{itemize}
...
\FigureEPS{h,t,b,p}{8.5cm}{./img/plateau2.eps}{Exemple de damier}{fig:damier}
...
Dans ce tableau :
\begin{itemize}
	\item[\textbullet] 0 correspond à une défaite du joueur en ligne
	\item[\textbullet] 1/2 correspond à un match nul
	\item[\textbullet] 1 correspond à une victoire du joueur en ligne
\end{itemize}
